...
<c:url var="listExaminationUrl" value="/admin-examination-list.html">
    <c:param name="urlType" value="url_list"/>
</c:url>
<c:url var="importUrl" value="/admin-examination-import.html">
    <c:param name="urlType" value="show_import_examination"/>
</c:url>
<html>
<head>
                        ...
                        <fmt:bundle basename="ResourcesBundle">
                            <display:table id="tableList" name="items.listResult" partialList="true" size="${items.totalItems}"
                                ...
                                <display:column title="<fieldset class='form-group'>
                                    ...
                                    <fieldset>
                                        <input type="checkbox" name="checkList" id="checkbox_${tableList.examinationId}" value="${tableList.examinationId}" class="check-box-element"/>
                                    </fieldset>
                                </display:column>
                                <display:column property="examinationId" titleKey="label.examination.id" sortable="true" sortName="examinationId"/>
                                ...
                                <display:column headerClass="col-actions" titleKey="label.action">
                                    <c:url var="editUrl" value="/ajax-admin-examination-edit.html">
                                        <c:param name="urlType" value="url_edit"/>
                                        <c:param name="pojo.examinationId" value="${tableList.examinationId}"/>
                                    </c:url>
                                    <a class="btn btn-sm btn-primary btn-edit" sc-url="${editUrl}" onclick="update(this)" data-toggle="tooltip" title="<fmt:message key='label.examination.edit' bundle='${lang}'/>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                    <a class="btn btn-sm btn-danger btn-cancel" data-toggle="tooltip" title="<fmt:message key='label.examination.delete' bundle='${lang}'/>"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </display:column>
                            </display:table>
                        </fmt:bundle>
                    </div>
...
<script>
    ...
    function update(btn) {
        ...
        if (typeof editUrl == 'undefined') {
            editUrl = '${editExaminationUrl}';
        }
        $('#myModal').load(editUrl,'', function () {
            $('#myModal').modal('toggle');
            addOrEditExamination();
        });
    }
    function addOrEditExamination() {
        $('#btnSave').click(function () {
            $('#editExaminationForm').submit();
        });
        $('#editExaminationForm').submit(function (e) {
            e.preventDefault();
