...
public class ExaminationController extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/views/web/examination/list.jsp").forward(request, response);
    }
    ...
    private void executeSearchExamination(HttpServletRequest request, ExaminationCommand command) {
        ...
        Object[] objects = SingletonServiceUtil.getExaminationServiceInstance().findByProperty(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getMaxPageItems());
