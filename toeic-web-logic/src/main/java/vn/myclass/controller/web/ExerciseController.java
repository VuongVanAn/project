...
public class ExerciseController extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/views/web/exercise/list.jsp").forward(request, response);
    }
    ...
    private void executeSearchExercise(HttpServletRequest request, ExerciseCommand command) {
        ...
        Object[] objects = SingletonServiceUtil.getExerciseServiceInstance().findByProperty(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getMaxPageItems());
